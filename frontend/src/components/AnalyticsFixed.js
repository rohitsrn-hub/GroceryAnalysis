                  ...
                  <CardContent className="text-center">
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-green-600">
                        {formatPercentage(abcAnalysis.summary.category_A?.percentage_items || 0)} of items
                      </p>
                      <p className="text-lg font-semibold text-green-700">
                        {formatIndianNumber(abcAnalysis.summary.category_A?.revenue || 0, true)}
                      </p>
                    </div>
                  </CardContent>
                </Card>
                  ...
                  <CardContent className="text-center">
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-blue-600">
                        {formatPercentage(abcAnalysis.summary.category_B?.percentage_items || 0)} of items
                      </p>
                      <p className="text-lg font-semibold text-blue-700">
                        {formatIndianNumber(abcAnalysis.summary.category_B?.revenue || 0, true)}
                      </p>
                    </div>
                  </CardContent>
                </Card>
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex flex-wrap gap-3 mt-2 text-xs text-gray-600">
                            <span className="flex items-center gap-1">
                              <strong>Stock:</strong> {Math.round(item.avg_closing_stock)} units
                            </span>
                            <span className="flex items-center gap-1">
                              <strong>Capital:</strong> {formatIndianNumber(item.capital_blocked, true)}
                            </span>
                            <span className="flex items-center gap-1">
                              <strong>Days to Sell:</strong> {item.days_to_sell === 9999 ? '∞' : Math.round(item.days_to_sell)}
                            </span>
                            <span className="flex items-center gap-1">
                              <strong>Monthly Sales:</strong> {item.avg_monthly_sales.toFixed(1)} units
                            </span>
                          </div>
                        </div>
                      </div>
                        ...
                        {inventoryAnalysis.dead_inventory?.slice(0, 5).map((item, index) => (
                          <div key={index} className="p-3 bg-red-50 rounded-lg border border-red-200">
                            ...
                            <div className="flex flex-wrap gap-3 mt-1 text-xs text-red-600">
                              <span><strong>Stock:</strong> {Math.round(item.avg_closing_stock || 0)} units</span>
                              <span><strong>Capital:</strong> {formatIndianNumber(item.capital_blocked || 0, true)}</span>
                              <span><strong>Days to Sell:</strong> {item.days_to_sell === 9999 ? '∞' : Math.round(item.days_to_sell || 0)}</span>
                            </div>
                          </div>
                      ...
                      <div className="space-y-3">
                        {inventoryAnalysis.slow_moving?.slice(0, 5).map((item, index) => (
                          <div key={index} className="p-3 bg-orange-50 rounded-lg border border-orange-200">
                            ...
                            <div className="flex flex-wrap gap-3 mt-1 text-xs text-orange-600">
                              <span><strong>Stock:</strong> {Math.round(item.avg_closing_stock || 0)} units</span>
                              <span><strong>Monthly Avg:</strong> {(item.avg_monthly_sales || 0).toFixed(1)} units</span>
                              <span><strong>Capital:</strong> {formatIndianNumber(item.capital_blocked || 0, true)}</span>
                            </div>
                          </div>
